...
def sleep_sort(arr: List[Union[int, float]]) -> List[Union[int, float]]:
    ...
    Sleep Sort is a quirky sorting algorithm that creates a thread for each element
    where the thread sleeps proportionally to the value of the element before adding 
    ...
    for num in arr:
        def worker(x):
            ...
            time.sleep(x * 0.001)  # Scale down sleep time 
